...
class MedicalSliceDataset(Dataset):
    def __init__(self, image_dir, label_dir=None, ids_list=None):
        """
        Initializes the dataset for loading CT image slices.

        Parameters:
        - image_dir: str, path to the directory containing images.
        - label_dir: str or None, path to the directory containing labels. If None, dataset operates in test mode.
        - ids_list: list or None, list of file IDs to include in the dataset.
        """
    ...
    # Defines a helper function to obtain individual slices from each CT scan file.
    def _get_slices(self):
        slices = []
        ids_set = set(self.ids_list) if self.ids_list else set(self.image_files)
        idx_r = 0
    ...
    # Retrieves a specific slice from the dataset and applies normalization. 
    # If label data is available, it returns both the image and label slices.
    def __getitem__(self, idx):
        ...
        if self.label_dir is not None:
            ...
            label_slice = torch.tensor(label_slice).unsqueeze(0)     # Shape: (1, H, W)

            return image_slice, label_slice
        else:
            return image_slice
    
    # Normalizes a CT image slice by rescaling intensity values to a [0, 1] range.
    @staticmethod
